...
const AddProductForm = () => {
  ...
  const handleSubmitWishList = async () => {
    ...
    try {
      const wishlistRes = await fetch("https://japan-pabuyanbackend2-0.onrender.com/wishlists", {
        method: "POST",
      ...
      console.log("RESPOSTA COMPLETA DO SERVIDOR:", wishlistData);
      const wishlistId = wishlistData.wishlistData.id;
      localStorage.setItem("wishlistId", wishlistId)
      console.log("ID da Wishlist criada:", wishlistId)


      const productlist = wishList.map((product) => (
        {
          name: product.name,
          status: "unseen",
      ...
      const res = await fetch("https://japan-pabuyanbackend2-0.onrender.com/requestedproducts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      //   const res = await fetch("http://localhost:3000/requestedproducts", {
      //     method: "POST",
      //     headers: { "Content-Type": "application/json" },
  ...
  {product.preview ? (
    ...
    <div className="w-full h-full flex items-center justify-center text-[10px] text-cor5">N/A</div>
                ...
                <button
                  onClick={() => setWishList(wishList.filter(thisProduct => thisProduct.temporaryId !== product.temporaryId))}
